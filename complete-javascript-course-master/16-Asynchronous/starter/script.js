...
/*
const getCountryAdjacent = function (name) {
  const request = new XMLHttpRequest();
  request.open('GET', `https://restcountries.eu/rest/v2/name/${name}`);
  request.send();
  request.addEventListener('load', function () {
...
/* 
In this challenge you will build a function 'whereAmI' which renders a country ONLY based on GPS coordinates. For that, you will use 
...
const whereAmI = function (lat, lng) {
  fetch(`https://geocode.xyz/${lat},${lng}?geoit=json`)
    .then(response => {
      if (!response.ok) throw new Error(`the api failed (${response.status})`);
      return response.json();
    })
    .then(data => {
...
4. After the 2 seconds have passed, hide the current image (set display to 'none'), and load a second image (HINT: Use the 
    image element returned by the createImage promise to hide the current image. You will need a global variable for that 😉);
...
const loadNPause = async function () {
  try {
    ...
    console.log('image 1 loaded');
    let wait1 = await wait(2);
    img.style.display = 'none';

    img = await createImage('img/img-2.jpg');
    console.log('image 2 loaded');
    let wait2 = await wait(2);
    img.style.display = 'none';
  } catch (err) {
...
const loadAll = async function (arr) {
  try {
    const imgs = arr.map(async element => await createImage(element));
