...
const calcDisplaySummary = function (account) {
  ...
  const int =
    (movements.filter(mov => mov > 0).reduce((acc, mov) => acc + mov, 0) *
      interest) /
    100;
  labelSumInterest.textContent = `${int > 1 ? int : 0}€`;
};
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    labelWelcome.textContent = `Error: invalid username or password`;
...
movements.forEach(function (a, index, array) {
  if (a > 0) {
    console.log(`You deposited ${a} on input ${index}`);
  } else {
    console.log(`You withdrew ${a} on input ${index}`);
  }
}); //break does not work in for each

currencies.forEach(function (curr, key, map) {
  console.log(`${key} is ${curr}`);
});

const setTest = new Set(['USD', 'EUR', 'GBP']);
setTest.forEach(function (curr, key, map) {
...
const checkDogs = function (dogsJulia, dogsKate) {
  const dogsJulia2 = dogsJulia.slice(1, 4);
...
const calcAverageHumanAge = function (arr) {
  ...
  const sum = adults.reduce(function (acc, curr, i, arr) {
    return acc + curr;
  }, 0);
...
dogs.forEach(function (element) {
  element.recFood = element.weight ** 0.75 * 28;
});
...
if (sarahDog.curFood > sarahDog.recFood) {
  ...
  console.log('sarahs dog is eating too litte');
} else {
  console.log('sarahs dog is eating well');
}
...
console.log(
  dogs.some(function (e) {
    return e.curFood === e.recFood;
  })
);

console.log(
  dogs.some(function (e) {
    return e.curFood <= e.recFood * 1.1 && e.curFood >= e.recFood * 0.9;
  })
);

const eatOk = dogs.filter(function (e) {
  return e.curFood <= e.recFood * 1.1 && e.curFood >= e.recFood * 0.9;
});
console.log(eatOk);
